...
enum class H264PayloadUnitType
{
    Unknown = 0,

    // RTP payload contains a complete NAL unit.
    SingleNalu,

    // RTP payload contains multiple complete NAL units.
    StapANalu,

    // RTP payload is one FU-A fragment.
    FuAFragment,
};
...
inline uint8_t H264GetNalType(uint8_t nal_header)
{
...
struct H264PayloadUnit
{
    ...
    // NAL type after parsing.
    uint8_t nal_type = 0;

    // For complete NAL unit:
    //   data = full NAL including NAL header.
    //
    // For FU-A fragment:
    //   data = fragment payload, excluding FU indicator and FU header.
    ...
    // FU-A reconstructed NAL header.
    // Valid only when unit_type == FuAFragment and fu_start == true.
...
struct H264AccessUnit
{
    ...
    // Complete NAL units belonging to one access unit.
    // Each item includes the NAL header.
