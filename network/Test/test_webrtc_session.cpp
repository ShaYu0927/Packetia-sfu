...
WebRtcMediaDescription Audio()
{
    ...
    media.mid = "audio";
    media.sdp.media = "audio";
    media.sdp.port = 9;
    media.sdp.proto = "UDP/TLS/RTP/SAVPF";
    media.sdp.fmts = {"111"};
    media.rtcpMux = true;
...
void Negotiation()
{
    ...
    offer.medias[0].sdp.attributes.push_back({"ssrc", "123 cname:REMOTE"});
    WebRtcMediaDescription data;
    data.mid = "data"; data.sdp.media = "application";
    data.sdp.port = 9; data.sdp.proto = "UDP/DTLS/SCTP"; data.sdp.fmts = {"webrtc-datachannel"};
    ...
    CHECK(answer.medias.size() == 2 && answer.sdp.medias.size() == 2);
    ...
    CHECK(answer.medias[0].sdp.GetAttribute("ice-pwd") == std::string(24, 'l'));
    CHECK(!answer.medias[0].sdp.HasAttribute("ssrc"));
    CHECK(answer.medias[1].sdp.port == 0);
    CHECK(answer.bundle.mids == std::vector<std::string>{"audio"});
    const auto id = answer.sdp.origin.sess_id;
    CHECK(f.session->CreateLocalAnswer(answer) && answer.sdp.origin.sess_id == id);
...
void BundleAndDirections()
{
    ...
    CHECK(answer.medias[1].sdp.port != 0 && !answer.medias[1].bundleOnly);
